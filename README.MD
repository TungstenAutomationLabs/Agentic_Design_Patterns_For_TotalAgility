...
- Tungsten TotalAgility 2026.1 or higher
  - User accounts **must** include an email address for the session remapping 
...
| Form | Purpose |
|-------|---------|
| **Reflection - Report Formatting Agent form** | Illustrates using the report generation agent, loading the generated report into a HTML control |
...
├── README.md                         # This file
├── dll-code/                           # MDCleaner.dll source code
│   └── Class1.cs                     # The code used in the  DLL included in the imported package (could be used as a script step if desired)
...
### Configuring the Tool Use - Chat Agent

The package includes an example agent called **Tool Use - Chat Agent**. To use it in a Generative AI chat control in a TotalAgility form, you must register the agent process as a **AI Agent** in TotalAgility:

1. Open **TotalAgility Advanced Studio**.
2. Navigate to **Integration → Generative AI**.
3. Add a new **AI Agent** entry and map it to the imported agent process.

![Adding_the_Tool_Use-Chat_Agent_as_an_Agent](./images/Agent_IO_Signature.png)

Once configured, the Tool Use - Chat Agent will be available for use in Generative AI chat controls.

## Configuring the Custom Services for use in TotalAgility Studio (optional)

To make the ```Get Session``` Custom Service in this package availabe TotalAgility Studio (formerly known as Quick Apps), you must enable the service for use. To do this: 

> In Advanced Studio, navigate to ```System > System Settings > TotalAgility Studio > Enabled for Use``` and add the ```Get Session``` Custom Service, then hit Save.
